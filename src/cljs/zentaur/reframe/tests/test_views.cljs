    ...
    (fn []
      [:div
       ...
       [:label {:for "subject_id"} " Select subject:"]
        (for [row-subject @(rf/subscribe [:subjects])]
          ^{:key (:id row-subject)} [:div.scheckbox (:subject row-subject) "  "  [:input {:type "checkbox" :title (:subject row-subject) :id (str "subjects_" (:id row-subject))
                                                                                          :on-change #(rf/dispatch [:add-search-elm {"subjects" (:id row-subject)}])}]])]
       [:div.scheckbox-container
        [:label {:for "level_id"} " Select level:"]
        (for [row-level @(rf/subscribe [:levels])]
          ^{:key (:id row-level)} [:div.scheckbox (:level row-level) "  " [:input {:type "checkbox" :title (:level row-level) :id (str "levels_" (:id row-level))
                                                                              :on-change #(rf/dispatch [:add-search-elm {"levels" (:id row-level)}])}]])]

       [:div.scheckbox-container
        [:label {:for "lang_id"} " Select lang:"]
        (for [row-lang @(rf/subscribe [:langs])]
          ^{:key (:id row-lang)} [:div.scheckbox (:lang row-lang) "  " [:input {:type "checkbox" :title (:lang row-lang) :id (str "langs_" (:id row-lang))
