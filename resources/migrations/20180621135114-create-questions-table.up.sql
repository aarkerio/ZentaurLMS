...
CREATE TABLE questions (
  ...
  tsv_en tsvector GENERATED ALWAYS AS (to_tsvector('english', coalesce(question, '') || ' ' || coalesce(hint, '') || ' ' || coalesce(explanation, ''))) STORED,
  tsv_es tsvector GENERATED ALWAYS AS (to_tsvector('spanish', coalesce(question, '') || ' ' || coalesce(hint, '') || ' ' || coalesce(explanation, ''))) STORED,
